...
public class MacroLineMarkerProvider implements LineMarkerProviderAdapter {
  ...
  @Override
  public void doCollectSlowLineMarkers(List<? extends PsiElement> elements, Collection<? super LineMarkerInfo<?>> result) {
    if (!enabled.getValue()) {
      return;
    }
  ...
  private static String getTooltip(List<GeneratedTarget> genTargets) {
    String targetRows =
        genTargets.stream()
            .sorted(com.google.idea.blaze.base.query.MacroLineMarkerProvider::compare)
            .map(MacroLineMarkerProvider::tooltipText)
            .collect(joining("<br>"));
